...
of regulatory relationships using either ScanBMA (the default
''algorithm'') or iterative Bayesian Model Averaging (Yeung
et~al. 2005, 2011) (by setting the parameter ''algorithm=iBMA''). The
...
expected network density as defined in (Lo et~al. 2011)).  The
...
in the BMA regression step.  Note that if ''algorithm" is set to ''ScanBMA", the
